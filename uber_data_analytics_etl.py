...
gcs_bucket_public_url = 'https://storage.googleapis.com/nischal_uber_de_project/uber_data.csv'

default_args = {
    'owner': 'NischalPaliwal',
    'retries': 2,
    'retry_delay': timedelta(minutes=4)
}

@dag(dag_id="uber_data_analytics",
     default_args=default_args,
     start_date=datetime(2025, 7, 27, 8),
...
def pipeline():
    ...
    @task()
    def load_uber_data():
        df = pd.read_csv(gcs_bucket_public_url)
        return df
    
    @task()
    def transform_uber_data(df):
    ...
    @task()
    def export_uber_data(data):
        
        service_account_json = Variable.get("gcp_service_account_key")
        ...
        for key, value in data.items():
            pandas_gbq.to_gbq(
                value, 
                destination_table=f'data-with-nischal.uber_analytics_dataset.{key}',
                project_id='data-with-nischal',
                credentials=credentials,
                if_exists='replace'
            )
    
    df = load_uber_data()
    transformed_data = transform_uber_data(df)
    export_uber_data(transformed_data)
